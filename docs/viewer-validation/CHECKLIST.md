...
Verified before asking: CDR reports removing the `vbaProject` content type, the macro-enabled `main+xml` content type, the `vbaProject` and `attachedTemplate` rels, and `ppt/vbaProject.bin`. **5 threat indicators in the input, 0 in the output, slide text preserved**, and both input and output load in LibreOffice. The generator asserts each of those, so a regression fails the build rather than quietly emitting a useless fixture.

> **Corrected on audit.** This first read "6 threat indicators" — a hardcoded literal in the generator that had drifted from the 5 tokens the probe actually checks, and was then quoted here and in two PR bodies as though it were measured. The count is now derived with `sum()`. Found alongside a worse one: `check_loads()` printed its verdict and both call sites *discarded* it, so "both input and output load in LibreOffice" was being reported, not enforced — a package no engine could open would still have produced a green run. Now asserted, and mutation-tested against an empty `_rels/.rels`.
