...
- [ ] **Resource names, IAM grants and alarms match across both paths** — `scripts/check_iac_parity.py` (run by CI) compares the `${prefix}-*` names, the IAM actions the Lambda role grants, and every alarm's metric/namespace/threshold/comparison. Two asymmetries are deliberate and encoded in the script: SAM creates the role, its inline policy and the EventBridge rule implicitly (no name to compare), and `S3WritePolicy` grants ACL actions the hand-written Terraform policy deliberately omits. The allow-lists are checked for staleness too — an entry that stops being needed fails the build rather than rotting. **Still manual:** bucket versioning/encryption/public-access-block, the TLS-only bucket policies, EventBridge pattern details, and Lambda tuning (memory/timeout/concurrency).
